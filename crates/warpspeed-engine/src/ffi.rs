...
use crate::engine::WarpSpeedEngine;
use crate::model::{CalcResult, EngineError, WorkbookSnapshot};

static ENGINE: OnceLock<Mutex<WarpSpeedEngine>> = OnceLock::new();

#[no_mangle]
pub extern "C" fn warpspeed_run_json(request_json: *const c_char) -> *mut c_char {
    let response = run_json(request_json).unwrap_or_else(error_response);
    string_to_ptr(response)
}

#[no_mangle]
pub extern "C" fn warpspeed_free_string(ptr: *mut c_char) {
    if ptr.is_null() {
        return;
    }
    unsafe {
...
fn run_json(request_json: *const c_char) -> Result<String, EngineError> {
    ...
    let engine = ENGINE.get_or_init(|| Mutex::new(WarpSpeedEngine::new()));
    let result = engine
        .lock()
