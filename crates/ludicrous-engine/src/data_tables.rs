...
/// suppresses validation against body values that are now LudicrousSpeed's own
...
fn evaluate_table_kernel(model: &Model<'_>, table: &DataTableRegion) -> ScenarioCounts {
    ...
    for ((actual, expected), scenario) in actual_values
        ...
        .zip(scenarios.iter())
    {
        // An override table's body holds LudicrousSpeed's own previous output, not
        ...
        if expected_grid_looks_stale(&expected_values) {
            ...
            let message = format!(
                "Excel's cached data table body is identical across all {} scenarios, which is not \
                 ...
                 they are slow); LudicrousSpeed's result is not validated against it, but the mismatch is \
                 more likely stale Excel data than a LudicrousSpeed error. {}",
                expected_values.len(),
