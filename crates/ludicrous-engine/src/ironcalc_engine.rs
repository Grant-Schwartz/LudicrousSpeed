...
fn apply_changed_cells(
    ...
    for change in changed_cells {
        ...
        // snapshot (LudicrousSpeed's own bookkeeping sheets being the obvious
    ...
    if !data_table_cells.is_empty() {
        ...
        notes.push(format!(
            "Rust computed {} data table output cells: {} matched Excel's cached values, {} \
             ...
             is already driven by LudicrousSpeed.",
...
fn benchmark_summary(
    ...
    let total_ludicrous_ms = started.elapsed().as_millis();
    let speedup_vs_excel = snapshot.excel_baseline_ms.and_then(|excel_ms| {
        if total_ludicrous_ms == 0 {
            None
        } else {
            Some(excel_ms as f64 / total_ludicrous_ms as f64)
        }
    });
...
/// override's body holds LudicrousSpeed's own output.
