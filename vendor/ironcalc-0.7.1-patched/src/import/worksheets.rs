...
pub(super) fn load_sheet<R: Read + std::io::Seek>(
    ...
    let mut shared_formula_lookup: HashMap<String, i32> = HashMap::new();
    // Built once per sheet instead of once per formula cell: constructing this
    // clones `defined_names`/`tables`, and on real workbooks with thousands of
    // defined names, doing that per formula cell was the dominant import cost.
    let mut formula_parser =
        new_parser_english(worksheets.to_owned(), defined_names, tables.clone());
                    ...
                    "shared" => {
                        ...
                        match fs[0].attribute("ref") {
                            Some(_) => {
                                ...
                                let mut parsed = formula_parser.parse(&formula, &cell_reference);
                                add_implicit_intersection(&mut parsed, true);
                                ...
                                match index_map.get(&si) {
                                    ...
                                    None => {
                                        ...
                                        match shared_formula_lookup.get(&formula) {
                                            // The formula is already present, use that index
                                            Some(index) => {
                                                formula_index = *index;
                                            }
                                            None => {
                                                shared_formulas.push(formula.clone());
                                                formula_index = shared_formulas.len() as i32 - 1;
                                                shared_formula_lookup.insert(formula, formula_index);
                                            }
                                        };
