...
namespace WarpSpeed.ExcelAddIn.Interop
{
    internal sealed class NativeEngineClient
    {
        private const string WindowsDll = "warpspeed_engine.dll";
        ...
        public EngineResponse Run(WorkbookSnapshot snapshot, out long nativeCallMs)
        {
            ...
            try
            {
                responsePtr = warpspeed_run_json(payload);
                stopwatch.Stop();
            ...
            finally
            {
                if (responsePtr != IntPtr.Zero)
                {
                    warpspeed_free_string(responsePtr);
                }
            }
        }
        ...
        private static extern IntPtr warpspeed_run_json(string requestJson);

        [DllImport(WindowsDll, CallingConvention = CallingConvention.Cdecl)]
        private static extern void warpspeed_free_string(IntPtr value);
    }
}
